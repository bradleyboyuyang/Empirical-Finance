...
def get_data(code, start_date, end_date) -> pd.DataFrame:
...
def feasible_region_drawing(df:pd.DataFrame, N):
    ...
    # 方法二：用dataframe.plot.scatter方法
    result = pd.DataFrame({'Returns':rets, 'Volatility':stds, 'Sharpe Ratio': sharpe_ratio})
    result.plot.scatter(x='Volatility', y='Returns', c='Sharpe Ratio', cmap='RdYlGn', edgecolors='black',marker='o')
...
if __name__ == '__main__':
    ...
    # 如果直接dataframe进行plot就自动带上colorbar，而如果是用的plt，那就要手动加上colorbar
